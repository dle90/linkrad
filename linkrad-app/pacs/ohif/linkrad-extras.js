...
(function () {
  ...
  // ----- Prefer JPEG 2000 transfer syntax for WADO-RS frame fetches -----
  // Probe on 2026-05-19 (probe-prod-transfer-syntax.js) showed prod Orthanc
  // is serving frames as uncompressed Explicit-VR-LE (~8.4MB / mammo frame).
  // When asked for transfer-syntax=1.2.840.10008.1.2.4.91 (JPEG 2000 lossless)
  // Orthanc transcodes on demand and returns the same frame at ~2.3MB —
  // ~3.5x bandwidth reduction with no infra change. Cornerstone3D ships a
  // JP2 decoder, so the response decodes correctly client-side.
  //
  // OHIF/cornerstone-wado-rs-loader's default Accept is just
  //   multipart/related; type="application/octet-stream"
  // which lets the server pick — Orthanc picks "source TS" = uncompressed.
  // We append `; transfer-syntax=1.2.840.10008.1.2.4.91` to force JP2.
  // QIDO/metadata requests use `application/dicom+json`, not multipart, so
  // they're not affected by this patch.
  var JP2_TS = '1.2.840.10008.1.2.4.91';
  function maybeUpgradeAccept(value) {
    if (typeof value !== 'string') return value;
    if (!/^multipart\/related/i.test(value)) return value;
    if (/transfer-syntax\s*=/i.test(value)) return value; // caller specified one
    return value + '; transfer-syntax=' + JP2_TS;
  }

  (function patchAcceptForJP2() {
    if (window._linkradJP2Patched) return;
    window._linkradJP2Patched = true;

    // fetch: cornerstone3D's newer wadors paths can use fetch directly.
        ...
        try {
          if (init && init.headers) {
            if (init.headers instanceof Headers) {
              var a = init.headers.get('Accept');
              var ua = maybeUpgradeAccept(a);
              if (ua !== a) init.headers.set('Accept', ua);
            } else if (typeof init.headers === 'object') {
              for (var k in init.headers) {
                if (k.toLowerCase() === 'accept') {
                  init.headers[k] = maybeUpgradeAccept(init.headers[k]);
                }
              }
            }
          }
        } catch (e) { /* don't break OHIF on header munging errors */ }
        return origFetch(input, init);
      };
    }

    // XHR: dicomweb-client (primary WADO-RS path in OHIF v3) uses XHR.
    if (window.XMLHttpRequest && XMLHttpRequest.prototype.setRequestHeader) {
      var origSet = XMLHttpRequest.prototype.setRequestHeader;
      XMLHttpRequest.prototype.setRequestHeader = function (name, value) {
        try {
          if (name && name.toLowerCase() === 'accept') {
            value = maybeUpgradeAccept(value);
          }
        } catch (e) {}
        return origSet.call(this, name, value);
      };
    }
    console.log('[LinkRad] JP2 Accept upgrader installed (transfer-syntax=' + JP2_TS + ')');
  })();
