...
- **Phân quyền vocabulary expanded** (2026-04-21) — permissions.js now defines 29 perms across 10 groups: added `consumables.record`, `reports.view`, `rad-reports.view`, `kpi-sales.view`, `referral.view`, `partners.manage`, `audit.view`. ROLE_CATALOG now has 10 named + 4 legacy roles (added `kinhdoanh`, kept `sale` as alias). Seed-hr.js was run against prod Atlas — RolePermission collection now carries the new defaults. **Route enforcement (Step 3) not yet done**: `/reports/*`, `/rad-reports/*`, `/catalogs/referral-doctors|partner-facilities|commission-*`, consumables update on Study, and `/audit-log` still gate on legacy role strings or `workflowOnly`. Migrate incrementally; sidebar is the fastest pass (see [Layout.jsx:86-116](linkrad-app/client/src/components/Layout.jsx#L86-L116)). Matrix display bug also fixed: view/manage columns now show "XEM"/"QL" verb prefix so they're visually distinguishable.
